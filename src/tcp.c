...
static int local_set_params(tcp_sock_t *p_self)
{
	int timeout;
...
int tcp_set_remote_timeout(tcp_sock_t *p_self, int p)
{
	if (p_self == NULL)
		return RC_INVALID_POINTER;
...
int tcp_get_port(tcp_sock_t *p_self, int *p)
{
	if (p_self == NULL)
		return RC_INVALID_POINTER;
