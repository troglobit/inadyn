...
static int do_parse_my_ip_address(ddns_t *ctx, int servernum)
{
	...
	if (ctx == NULL || ctx->http_transaction.rsp_len <= 0 || ctx->http_transaction.p_rsp == NULL)
	{
		return RC_INVALID_POINTER;
	}
...
int dyn_dns_main(ddns_t *ctx, int argc, char* argv[])
{
			    ...
			    strcmp(ctx->info[i].system->key, "ipv6tb@he.net"))
			{
				...
				if (!(s = getaddrinfo(ctx->info[i].alias[0].names.name, NULL, &hints, &result)))
				{
					...
					if (!getnameinfo(result->ai_addr, result->ai_addrlen, name, sizeof(name), NULL, 0, NI_NUMERICHOST))
					{
						...
						strncpy(ctx->info[i].my_ip_address.name, name, sizeof(ctx->info[i].my_ip_address.name));
						logit(LOG_INFO, "Resolving hostname %s => IP# %s", ctx->info[i].alias[0].names.name, name);
					}
					freeaddrinfo(result);
				}
				else
				{
					logit(LOG_WARNING, "Failed resolving hostname %s: %s", ctx->info[i].alias[0].names.name, gai_strerror(s));
				}
			}
		}
	}
	...
	do
	{
		...
		while (1)
		{
			...
			ctx->sleep_sec = rc == RC_DYNDNS_RSP_RETRY_LATER ? ctx->error_update_period_sec : ctx->normal_update_period_sec;
